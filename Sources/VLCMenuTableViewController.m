...
- (void)viewDidLoad
{
    ...
    _menuItemsSectionTwo = @[@"Browser",@"LOCAL_NETWORK", @"OPEN_NETWORK", @"DOWNLOAD_FROM_HTTP", @"WiFi Upload", @"Dropbox", @"Google Drive"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (section == 0) {
        ...
        else if ([rawTitle isEqualToString:@"Browser"])
            cell.imageView.image = [UIImage imageNamed:@"OpenWWW"];
...
{
    [((VLCAppDelegate*)[[UIApplication sharedApplication] delegate]).revealController incAdCounter];
